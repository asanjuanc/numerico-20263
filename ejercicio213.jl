...
# ╔═╡ 56c71da0-58bd-4ed2-bd68-add95f11ddd3
begin 
	using Polynomials
	using Plots
end

# ╔═╡ 2ff5d732-a547-11f1-bea7-2fa4136aa488
begin 
	años = [1990, 2000, 2010, 2020]
	usa = [252.120,	281.711, 309.011, 331.003]
	india = [873.278,	1056.576, 	1234.281, 1380.004]
	polonia = [37.960, 	38.557, 	38.330, 37.847]
	tiempo = (años .- 1990)/10
end

# ╔═╡ b1a9b0f2-98b1-40a7-b08a-c5684499890b
md""" 
# Un problema de Demografía con datos de los años $(años[begin]) al $(años[end]) con datos de cada 10 años
"""

# ╔═╡ 09683613-1ba8-423c-b6e9-fc3f90f43c82
md" obtenemos la matriz de Vandermonde primero con este código"

# ╔═╡ 2cd27c6b-1c9c-4a02-9b93-775838875103
vander = [tiempo[i]^j for i in 1:4 ,  j in 0:3]

# ╔═╡ 54b2166f-d011-4d58-8ba6-dd1d2923d785
md"Tenemos que resolver el sitema $V c = y$, donde $V$ es la matriz de Vandermonde, $c$  es lo que queremos encontrar, los coeficientes del polinimo y $y$ la poplacion de cada pais"

# ╔═╡ 0f924966-e2d7-4f8a-b374-a20e8445f496
begin
	cusa = vander \ usa 
	cindia = vander \ india 
	cpolonia = vander \ polonia
end

# ╔═╡ c4f1e8e4-cfe7-4d63-ac2b-0dc8f1495ffd
md" Para obtener los polinimio usamos el paquete Poylinomials"

# ╔═╡ c72e908d-8a28-4699-8fb7-a42d9b8bf5e6
begin 
	pusa = Polynomial(cusa)
	ppolonia = Polynomial(cpolonia)
	pindia = Polynomial(cindia)
end

# ╔═╡ 4d59ca45-4e89-4b2c-aad0-0329b3a8a01b
#verifiquemos
pindia(2) 

# ╔═╡ 2270003e-51fe-4aa9-8669-320d51d6ae42
begin
	scatter(tiempo, polonia, label="Muestra", title="Interpolación demográfica", xlabel="Años", ylabel="Poblacion (en millones de habitantes)")
	plot!(x->ppolonia(x), 0,3)
end

# ╔═╡ 98edc6ba-cde4-433d-9ac8-dc10e3ff006c
pder = derivative(ppolonia)

# ╔═╡ 3f382f04-d6e4-424b-8503-92aeda4cbe73
roots(pder)

# ╔═╡ caf5794b-2b5d-41fe-b156-ee5439b1a028
1.11*10+1990
...
manifest_format = "2.0"
project_hash = "482cf8a49874fdee3e034cba244359ef53305a6b"
